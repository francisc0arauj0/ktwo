...
void initIDT() {
	...
	// First chip  -  0x20 commands and 0x21 data
	// Second chip -  0xA0 commands and 0xA1 data
	outPortB(0x20, 0x11);
	outPortB(0xA0, 0x11);
	...
	// setIdtGate(30, (uint32_t)isr128, 0x08, 0x8E);  syscalls
	// setIdtGate(31, (uint32_t)isr177, 0x08, 0x8E);  syscalls
...
void setIdtGate(uint8_t number, uint32_t base, uint16_t ss, uint8_t flags) {
	idt_entries[number].base_low = base & 0xFFFF;
	idt_entries[number].base_height = (base >> 16) & 0xFFFF;
	idt_entries[number].ss = ss;
...
void irq_handler_manager(int irq, void (*handler)(struct InterruptRegisters *r), int install) {
	if (install) {

...
void initGDT() {
	...
	setGdtGate(1, 0, 0xFFFFFFFF, 0x9A, 0xCF); // Kernel code segment (r0)
	setGdtGate(2, 0, 0xFFFFFFFF, 0x92, 0xCF); // Kernel data segment (r0)
	setGdtGate(3, 0, 0xFFFFFFFF, 0xFA, 0xCF); // User code segment   (r3)
	setGdtGate(4, 0, 0xFFFFFFFF, 0xF2, 0xCF); // User data segment   (r3)
...
void writeTSS(uint32_t number, uint16_t ss0, uint32_t esp0) {
	...
	setGdtGate(number, base, limit, 0x89, 0x00);
